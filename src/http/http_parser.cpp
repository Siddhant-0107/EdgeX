...
bool equals_case_insensitive_ascii(std::string_view lhs, std::string_view rhs) {
    ...
    for (std::size_t i = 0; i < lhs.size(); ++i) {
        if (to_lower_ascii(lhs[i]) != to_lower_ascii(rhs[i])) return false;
    }
    return true;
}
...
std::optional<HttpMethod> parse_method(std::string_view token) {
    ...
    if (token == "DELETE") return HttpMethod::Delete_;
...
std::optional<std::size_t> parse_content_length_value(std::string_view raw) {
    ...
    for (char ch : value) {
        ...
        if (result > (max_size - digit) / 10) return std::nullopt;
...
ParseResult HttpParser::feed(std::string_view chunk) {
    if (state_ == State::Error) return make_error_result();
...
bool HttpParser::has_error() const noexcept { return state_ == State::Error; }

bool HttpParser::is_waiting_for_more() const noexcept {
    return state_ == State::RequestLine || state_ == State::Headers || state_ == State::Body;
}

std::size_t HttpParser::buffered_bytes() const noexcept { return buffer_.size(); }
...
ParseResult HttpParser::parse_available_data() {
    while (true) {
        if (state_ == State::RequestLine) {
            ...
            if (!parse_request_line(line_end)) return make_error_result();
        ...
        if (state_ == State::Headers) {
            ...
            if (expected_body_bytes_ == 0) {
                if (content_length_present_) working_request_.body = std::vector<std::uint8_t>{};
                if (!buffer_.empty()) {
                    set_error(ParseErrorCode::UnexpectedDataAfterRequest,
                              "unexpected bytes after complete request", 0);
                    return make_error_result();
        ...
        if (state_ == State::Body) {
            if (buffer_.size() < expected_body_bytes_) return make_incomplete_result();
            if (buffer_.size() > expected_body_bytes_) {
                set_error(ParseErrorCode::UnexpectedDataAfterRequest,
                          "unexpected bytes after complete request body",
                          expected_body_bytes_);
                return make_error_result();
            }

            working_request_.body = std::vector<std::uint8_t>(
                reinterpret_cast<const std::uint8_t*>(buffer_.data()),
                reinterpret_cast<const std::uint8_t*>(buffer_.data()) + expected_body_bytes_);
            buffer_.clear();
            completed_request_ = working_request_;
            state_ = State::Complete;
            return make_complete_result();
        ...
        if (state_ == State::Complete) return make_complete_result();
        return make_error_result();
    }
}
...
bool HttpParser::parse_request_line(std::size_t line_end) {
    ...
    const std::string_view target_token = line.substr(first_space + 1, second_space - first_space - 1);
    const std::string_view version_token = line.substr(second_space + 1);

    const auto method = parse_method(method_token);
    if (!method) {
    ...
    const auto version = parse_version(version_token);
    if (!version) {
    ...
    while (current < headers_end) {
        ...
        if (next == std::string::npos || next > headers_end) {
            set_error(ParseErrorCode::InvalidHeaderLine,
                      "malformed header line terminator", line_start);
            return false;
        }
        ...
        if (line.empty()) {
            set_error(ParseErrorCode::InvalidHeaderLine,
                      "empty header line inside header block", line_start);
            return false;
        }
        ...
        if (colon == std::string_view::npos || colon == 0) {
            set_error(ParseErrorCode::InvalidHeaderLine,
                      "header must contain name:value", line_start);
            return false;
        }
        ...
        for (std::size_t i = 0; i < name.size(); ++i) {
            if (!is_tchar(name[i])) {
                set_error(ParseErrorCode::InvalidHeaderLine,
                          "invalid header name character", line_start + i);
                return false;
            }
        }
        ...
        for (std::size_t i = 0; i < value.size(); ++i) {
            if (!is_visible_field_value_char(value[i])) {
                set_error(ParseErrorCode::InvalidHeaderLine,
                          "invalid header value character",
                          line_start + colon + 1 + i);
                return false;
            }
        }
...
bool HttpParser::prepare_body_expectation() {
    ...
    for (const HeaderField& header : working_request_.headers) {
        if (equals_case_insensitive_ascii(header.name, "Transfer-Encoding")) {
            set_error(ParseErrorCode::UnsupportedTransferEncoding,
                      "Transfer-Encoding is not supported", 0);
            return false;
        }

        if (!equals_case_insensitive_ascii(header.name, "Content-Length")) continue;
        content_length_present_ = true;
        const auto parsed = parse_content_length_value(header.value);
        if (!parsed) {
            set_error(ParseErrorCode::InvalidContentLength,
                      "invalid Content-Length value", 0);
            return false;
        }
        if (declared_content_length && *declared_content_length != *parsed) {
            set_error(ParseErrorCode::InvalidContentLength,
                      "conflicting Content-Length headers", 0);
            return false;
        }
    ...
    if (expected_body_bytes_ > limits_.max_body_bytes) {
        set_error(ParseErrorCode::BodyTooLarge,
                  "body exceeds max_body_bytes", 0);
        return false;
    }
    return true;
}
...
bool HttpParser::finalize_body_if_ready() {
    if (buffer_.size() < expected_body_bytes_) return false;
    if (buffer_.size() > expected_body_bytes_) {
        set_error(ParseErrorCode::UnexpectedDataAfterRequest,
