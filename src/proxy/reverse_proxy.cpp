...
bool is_hop_by_hop_header(std::string_view name) noexcept {
    return equals_ci(name, "Connection") || equals_ci(name, "Keep-Alive") ||
           equals_ci(name, "Proxy-Authenticate") || equals_ci(name, "Proxy-Authorization") ||
           equals_ci(name, "TE") || equals_ci(name, "Trailer") ||
           equals_ci(name, "Transfer-Encoding") || equals_ci(name, "Upgrade");
}
...
std::string serialize_request(const http::HttpRequest& request, const Backend& backend) {
    ...
    std::string result = method_to_string(request.method) + " " + request.target + " HTTP/1.1\r\n";
    bool has_host = false;
    bool has_content_length = false;

    for (const auto& header : request.headers) {
        if (is_hop_by_hop_header(header.name)) continue;
        if (equals_ci(header.name, "Host")) has_host = true;
        if (equals_ci(header.name, "Content-Length")) has_content_length = true;
    ...
    if (body_size != 0 && !has_content_length) {
        result += "Content-Length: " + std::to_string(body_size) + "\r\n";
    }
...
std::size_t find_header_end(const std::vector<std::uint8_t>& data) {
    ...
    return it == data.end() ? std::string::npos
...
http::HttpResponse parse_response(const std::vector<std::uint8_t>& data) {
    ...
    if (header_end == std::string::npos) throw std::runtime_error("incomplete upstream response headers");

    const std::string header_text(reinterpret_cast<const char*>(data.data()), header_end);
    std::istringstream stream(header_text);
    std::string status_line;
    if (!std::getline(stream, status_line)) throw std::runtime_error("missing upstream status line");
    ...
    while (std::getline(stream, line)) {
        ...
        if (colon == std::string::npos || colon == 0) throw std::runtime_error("malformed upstream header");

        std::string name = line.substr(0, colon);
        std::string value = trim(line.substr(colon + 1));
        if (is_hop_by_hop_header(name)) continue;

        if (equals_ci(name, "Content-Length")) {
            try {
                const unsigned long long parsed = std::stoull(value);
                if (parsed > std::numeric_limits<std::size_t>::max()) throw std::runtime_error("too large");
    ...
    if (body_start > data.size()) throw std::runtime_error("invalid upstream response");
    const std::size_t available = data.size() - body_start;

    if (has_content_length) {
        if (available < content_length) throw std::runtime_error("incomplete upstream response body");
...
const ReverseProxyConfig& ReverseProxy::config() const noexcept { return config_; }
